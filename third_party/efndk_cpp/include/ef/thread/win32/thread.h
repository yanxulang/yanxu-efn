...
namespace EF_Thread
{
	...
	{
	private:
		IMutex& getMutex() { static Mutex m; return m; }
		ISemaphore& getSemaphore()  { static Semaphore s; return s; }
	...
	public:
		...
		int create(int nStackSize = 0)
		{
			...
			m_handle = ::CreateThread(NULL, nStackSize, (LPTHREAD_START_ROUTINE)ThreadMainHandler, this, 0, NULL);
		...
		int wait(void)
		{
			DWORD ret = ::WaitForSingleObject((HANDLE)m_handle, INFINITE);
			...
			if(ret == WAIT_TIMEOUT) return EAGAIN;
			return EINVAL;
		}

		int terminate()
		{
			return ::TerminateThread((HANDLE)m_handle, 0) ? 0 : EINVAL; 
		}
